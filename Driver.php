...
// connecting to database
    ...
    //  if the driver is ready for the ride $flag=2
     if($row['is_active']){$_SESSION['cloc']=$row['current_location'];$flag=2;}
     else{$flag=1;}//flag 1 if he is offline
     ...
     if(isset($_GET['live']) && $_GET['live']=='live'){
        ...
        if($stmt->execute()){ header("Location: " . $_SERVER['PHP_SELF']);}//reloads the file after updating table}
     }
...
if(isset($_GET['accept'])){
  $sql = "UPDATE bookings SET status = 'accepted' WHERE driver_id = ?";
        $stmt = $conn->prepare($sql); 
        $stmt->bind_param("i", $_SESSION['did']); 
        if($stmt->execute()){ 
...
<h1 class="TagLine">BE THE <span class='ride'>DIFFERENCE</span> ON EVERY ROAD</h1>
  <div class="cardCont <?php if($flag!=1){echo 'display';}?>" >
        ...
        <div class="searchcard-content">
          ...
          <form method="get" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
            ...
            <div class="form-group">
                ...
                <input type="text" name="cloc" id="autocomplete" id="from" placeholder="Current location" required>
      ...
      if ($flag == 2 && isset($_SESSION['passWaitFlag'])) {
          ...
          <div class="cardCont">
              <div class="searchcard confirmcard">
                  ...
                  <div class="searchcard-content">
                      ...
                      <p class="confirm-details">The trip is from <?php echo $row['from'] . ' to ' . $row['to']; ?></p>
                      <form method="get" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                          <div class="fromTo">
                              <div class="form-group">
          ...
          if (isset($_GET['completed'])) {
              if ($_GET['otp'] == $_SESSION['otp']) {
                  $sql = "UPDATE bookings SET status = 'completed' WHERE driver_id = ? AND status = 'accepted'";
                  $stmt = $conn->prepare($sql);
                  $stmt->bind_param("i", $_SESSION['did']);
                  if ($stmt->execute()) {
                      // Unset the session flag and redirect
      ...
      if ($flag == 2 && !isset($_SESSION['passWaitFlag'])) {
          ...
          <div class="onlive">
              <form method="get" style="display:inline;" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                  <button value="true" name="change" class="change search"><span>CHANGE LOCATION</span></button>
                  ...
                  while ($row = $result->fetch_assoc()) { ?>
                      <div class="card">
                          <div class="card-content">
                              <h3 >From: <?php echo $row['from']; ?> To: <?php echo $row['to']; ?></h3>
                              <div class="button-group">
                                  <form method="get" style="display:inline;" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                                      <button name="delete" value='<?php echo $row["pass_id"]; ?>'>Cancel</button>
